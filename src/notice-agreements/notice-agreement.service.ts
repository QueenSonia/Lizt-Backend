...
export class NoticeAgreementService {
  ...
  create = async (dto: CreateNoticeAgreementDto) => {
  ...
  resendNoticeAgreement = async (id: string) => {
  ...
  getNoticeAgreementsByTenantId = async (
    tenant_id: string,
    queryParams: NoticeAgreementFilter,
  ) => {
    const page = queryParams?.page
  ...
  getNoticeAnalytics = async (id: string) => {
  ...
  attachNoticeDocument = async (property_id: string, fileUrls: string[]) => {
    try {
      const property = await this.propertyRepo.findOne({
