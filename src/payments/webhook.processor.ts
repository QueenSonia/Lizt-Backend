...
export class WebhookProcessor {
  ...
  async handleWebhookEvent(job: Job<WebhookJobData>): Promise<void> {
    ...
    this.logger.log(`Processing background webhook event: ${event} for reference: ${data.reference}`);

    try {
      if (event === 'charge.success') {
        await this.paymentService.processSuccessfulPayment(data);
        this.logger.log(`Successfully processed charge.success for reference: ${data.reference}`);
      } else {
        this.logger.warn(`Received unhandled event type in background: ${event}`);
      }
    } catch (error) {
      ...
      this.logger.error(`Error processing webhook job: ${error.message}`, error.stack);
