...
export class ReceiptGeneratorService {
  ...
  generateReceiptHTML(receipt: Receipt): string {
    ...
    const companyName =
      branding?.businessName || branding?.companyName || 'Property Kraft';
    const companyAddress =
      branding?.businessAddress ||
      branding?.address ||
      '17 Ayinde Akinmade Street, Lekki Phase 1, Lagos, Nigeria.';
    const companyEmail = branding?.contactEmail || branding?.email || '';
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .company-info { flex: 1; }
    .company-name { font-size: 14px; font-weight: 600; color: #111; margin-bottom: 4px; }
    .company-detail { font-size: 12px; color: #6b7280; line-height: 1.6; }
    .receipt-title { text-align: right; }
    .receipt-title h1 { font-size: 24px; font-weight: 700; color: #111; letter-spacing: -0.5px; margin-bottom: 12px; }
    ...
    .summary-amount { font-size: 36px; font-weight: 700; color: #111; margin-bottom: 8px; }
    .summary-label { font-size: 18px; font-weight: 500; color: #374151; margin-bottom: 12px; }
    .summary-details { font-size: 14px; color: #6b7280; }
    .summary-details .paid { font-weight: 500; color: #15803d; }
    .summary-details .ref { font-weight: 500; color: #111; }
  ...
  <div class="container">
    <div class="header">
      <div class="company-info">
        ...
        <p class="company-detail">${this.escapeHtml(companyAddress)}</p>
        ${companyEmail ? `<p class="company-detail" style="margin-top:4px">${this.escapeHtml(companyEmail)}</p>` : ''}
      </div>
    ...
    <div class="summary-block">
      ...
      <div class="summary-details">
        <span>Status: </span><span class="paid">PAID</span>
        ${receipt.payment_method ? ` &nbsp;|&nbsp; <span>Method: </span><span class="ref">${this.escapeHtml(receipt.payment_method)}</span>` : ''}
        &nbsp;|&nbsp; <span>Ref: </span><span class="ref">${this.escapeHtml(receipt.payment_reference)}</span>
      </div>
    </div>

    <div style="margin-bottom:40px">
      <h3 class="section-title">Payment Details</h3>
      ${receipt.invoice_number ? `<div class="detail-row"><span class="detail-label">Invoice Reference</span><span class="detail-value">${this.escapeHtml(receipt.invoice_number)}</span></div>` : ''}
      <div class="detail-row"><span class="detail-label">Amount Paid</span><span class="detail-value" style="font-weight:600">${formatCurrency(receipt.amount_paid)}</span></div>
      ${receipt.notes ? `<div class="detail-row"><span class="detail-label">Notes</span><span class="detail-value" style="text-align:right;max-width:400px">${this.escapeHtml(receipt.notes)}</span></div>` : ''}
    </div>
    ...
    <div class="info-grid">
      <div>
        ...
        <div class="info-item"><p class="info-label">Name</p><p class="info-value">${this.escapeHtml(receipt.tenant_name)}</p></div>
      ...
      <div>
        ...
        <div class="info-item"><p class="info-label">Property Name</p><p class="info-value">${this.escapeHtml(receipt.property_name)}</p></div>
